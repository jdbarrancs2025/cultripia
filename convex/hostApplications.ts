...
export const createApplication = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.experienceDescription.length > 1000 || args.experienceDescription.length < 20) {
      throw new Error("Experience description must be between 20 and 1000 characters")
    }
    ...
    const validLanguages = ["spanish", "english", "french", "german", "italian", "portuguese", "kiche", "kaqchikel"];
    const invalidLanguages = args.languagesSpoken.filter(lang => !validLanguages.includes(lang));
    if (invalidLanguages.length > 0) {
      throw new Error("Invalid language selection")
    }

    // Validate location
    const validLocations = ["antigua", "lake-atitlan", "chichicastenango", "tikal", "semuc-champey", "guatemala-city", "quetzaltenango", "livingston"];
    if (!validLocations.includes(args.location)) {
      throw new Error("Invalid location selection")
    }

    // Validate experience type
    const validTypes = ["cooking", "artisanal", "cultural", "nature", "historical", "spiritual", "agricultural", "textile"];
    ...
    if (existingApplication) {
      if (existingApplication.status === "pending") {
        throw new Error("You already have a pending host application. Please wait for review.")
      } else {
